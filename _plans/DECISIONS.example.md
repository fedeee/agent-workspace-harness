...
renumber.
Require scope, evidence, and a reconsideration condition. The evidence below is fictional.
...
- **Scope**: Budget display in the example purchase flow.
- **Evidence**: Example usability trial: three controls caused unused capacity.
- **Reconsider when**: Separate paid resources require independent accounting.
...
- **Decision**: Do not add a second user-visible budget number.

## Classifier

### D2 — Hardcoded keyword lists in engine source
- **Status**: settled (example)
- **Scope**: Example classifier on two catalogue slices.
- **Evidence**: Example cycle: false positives rose on the second slice.
- **Reconsider when**: A new frozen benchmark shows generalization across slices.
...
- **Decision**: Do not put class names, category lists, or brand lists in engine code.
...
- **Status**: settled (example)
- **Scope**: Example classifier prompt change on a frozen gold set.
- **Evidence**: Example A/B result: false omission rate rose despite one improved case.
- **Reconsider when**: A measured candidate meets both error criteria.
...
- **Decision**: Do not keep a change unless precision error drops and recall error does not rise.

## Search

### D4 — Second read model for the same list
- **Status**: settled (example)
- **Scope**: Example list page with the existing primary-store query.
- **Evidence**: Example consistency test: primary and index results diverged.
- **Reconsider when**: Measured query latency requires another index and consistency tests pass.
...
- **Decision**: Do not add a second read path for the same list until a cycle
  keeps it.
